(()=>{var h={reply:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 18v-2a4 4 0 0 0-4-4H4"/><path d="m9 17-5-5 5-5"/></svg>',share:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg>',user:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',mail:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>',send:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/></svg>',edit:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>'};var S=document.currentScript,M=S?new URL(S.src).origin:window.location.origin;function _(d){let t=parseInt(d.replace("#",""),16);return[t>>16&255,t>>8&255,t&255]}function k(d,t,s){return"#"+[d,t,s].map(i=>Math.round(Math.max(0,Math.min(255,i))).toString(16).padStart(2,"0")).join("")}function L(d,t,s){d/=255,t/=255,s/=255;let i=Math.max(d,t,s),e=Math.min(d,t,s),a,n,o=(i+e)/2;if(i===e)a=n=0;else{let r=i-e;switch(n=o>.5?r/(2-i-e):r/(i+e),i){case d:a=((t-s)/r+(t<s?6:0))/6;break;case t:a=((s-d)/r+2)/6;break;default:a=((d-t)/r+4)/6}}return[a*360,n,o]}function T(d,t,s){d/=360;let i=(n,o,r)=>(r<0&&(r+=1),r>1&&(r-=1),r<1/6?n+(o-n)*6*r:r<1/2?o:r<2/3?n+(o-n)*(2/3-r)*6:n);if(t===0){let n=Math.round(s*255);return[n,n,n]}let e=s<.5?s*(1+t):s+t-s*t,a=2*s-e;return[i(a,e,d+1/3),i(a,e,d),i(a,e,d-1/3)].map(n=>Math.round(n*255))}function E(d,t){if(!d)return null;if(d=d.trim(),d.startsWith("var(")||d.startsWith("--")){let a=d.startsWith("var(")?d.slice(4,-1).split(",")[0].trim():d,n=document.createElement("span");n.style.cssText="position:fixed;left:-9999px;top:-9999px;color:var("+a+")",t.appendChild(n);let o=getComputedStyle(n).color;return t.removeChild(n),o?E(o,t):null}if(/^#[0-9a-fA-F]{6}$/.test(d))return d;if(/^#[0-9a-fA-F]{3}$/.test(d)){let[,a,n,o]=d.match(/^#(.)(.)(.)$/);return`#${a}${a}${n}${n}${o}${o}`}let s=d.match(/^rgba?\(\s*([\d.]+)[,\s]+([\d.]+)[,\s]+([\d.]+)/i);if(s)return k(+s[1],+s[2],+s[3]);let i=d.match(/^hsla?\(\s*([\d.]+)[,\s]+([\d.]+)%[,\s]+([\d.]+)%/i);if(i){let[a,n,o]=T(+i[1],+i[2]/100,+i[3]/100);return k(a,n,o)}let e=d.match(/^oklch\(\s*([\d.]+%?)[,\s]+([\d.]+)[,\s]+([\d.]+)/i);if(e){let a=parseFloat(e[1]);e[1].includes("%")&&(a/=100);let n=parseFloat(e[2]),o=parseFloat(e[3])*Math.PI/180,r=n*Math.cos(o),c=n*Math.sin(o),l=a+.3963377774*r+.2158037573*c,p=a-.1055613458*r-.0638541728*c,f=a-.0894841775*r-1.291485548*c,y=l**3,m=p**3,u=f**3,$=4.0767416621*y-3.3077115913*m+.2309699292*u,b=-1.2684380046*y+2.6097574011*m-.3413193965*u,g=-.0041960863*y-.7034186147*m+1.707614701*u,v=C=>C<=.0031308?12.92*C:1.055*C**(1/2.4)-.055;return k(Math.round(Math.max(0,Math.min(1,v($)))*255),Math.round(Math.max(0,Math.min(1,v(b)))*255),Math.round(Math.max(0,Math.min(1,v(g)))*255))}return null}function B(d){let[t,s,i]=_(d),[e,a]=L(t,s,i),n={50:.96,100:.93,200:.86,300:.74,400:.6,500:.48,600:.38,700:.3,800:.22,900:.15},o={};for(let[r,c]of Object.entries(n)){let[l,p,f]=T(e,Math.min(a,.85),c);o[r]=k(l,p,f)}return o}var x=class{constructor(t){window.DiscussWidgetInstance=this,this.container=t.container,this.container&&(this.postUrl=t.postUrl||this.container.dataset.url||window.location.pathname,this.serverUrl=t.serverUrl||M,this.fetchUrl=t.fetchUrl||`${this.serverUrl}/api/comments?post_url=${encodeURIComponent(this.postUrl)}`,this.configUrl=t.configUrl||`${this.serverUrl}/api/comments/config`,this.config={},this.primaryColor=t.primaryColor||null,this.domainId=t.domainId||null,this.title=t.title??"Leave a comment",this.placeholder=t.placeholder??"Share your thoughts... (*markdown* supported)",this.darkSelector=t.darkSelector||null,this.icons={name:t.icons?.name!==void 0?t.icons.name:h.user,email:t.icons?.email!==void 0?t.icons.email:h.mail,submit:t.icons?.submit!==void 0?t.icons.submit:h.send},this.editTokens=new Map,this._rawContent=new Map,this.init=this.init.bind(this),this.render=this.render.bind(this),this.renderComment=this.renderComment.bind(this),this.renderForm=this.renderForm.bind(this),this.handleSubmit=this.handleSubmit.bind(this),this.primaryColor&&this.applyTheme(this.primaryColor),this.watchPrimaryColor(),this.init())}applyTheme(t){let s=E(t,this.container);if(!s)return;let i=B(s),e=this.container;e.style.setProperty("--brand-50",i[50]),e.style.setProperty("--brand-100",i[100]),e.style.setProperty("--brand-200",i[200]),e.style.setProperty("--brand-300",i[300]),e.style.setProperty("--brand-400",i[400]),e.style.setProperty("--brand-500",i[500]),e.style.setProperty("--brand-600",i[600]),e.style.setProperty("--brand-700",i[700]),e.style.setProperty("--brand-800",i[800]),e.style.setProperty("--brand-900",i[900]),e.style.setProperty("--accent-fg",i[700]),e.style.setProperty("--accent-surface",i[50]),e.style.setProperty("--focus-ring",i[700])}watchPrimaryColor(){if(!this.primaryColor||!this.darkSelector)return;let t=this.primaryColor.trim();if(!t.startsWith("var(")&&!t.startsWith("--"))return;let s=()=>this.applyTheme(this.primaryColor),i=e=>{e&&new MutationObserver(s).observe(e,{attributes:!0,attributeFilter:["class"]})};i(document.documentElement),i(document.body)}injectDarkStyles(){if(!this.darkSelector)return;let t="discuss-dark-style";if(document.getElementById(t))return;let s=document.createElement("style");s.id=t,s.textContent=`${this.darkSelector} #discuss-comments {
        ...
        `}getAdminBadges(t){return""}getAdminTooltip(t){return""}getAdminControls(t){return""}getEditButton(t){return this.editTokens.has(t.id)?`<button class="discuss-action-btn discuss-edit-btn" data-id="${t.id}">${h.edit} <span>Edit</span></button>`:""}_startInlineEdit(t){let s=document.getElementById(`discuss-collapse-target-${t}`);if(!s)return;let i=s.querySelector(".discuss-comment-body"),e=s.querySelector(".discuss-flex.discuss-gap-2");if(!i||!e)return;let a=i.innerHTML,n=e.innerHTML,o=this._rawContent.get(t)||"",r=document.createElement("textarea");r.className="discuss-form-textarea",r.style.marginBottom="0.5rem",r.value=o,i.innerHTML="",i.appendChild(r);let c=document.createElement("button");c.className="discuss-btn discuss-btn-primary",c.style.cssText="font-size:0.8125rem;padding:0.25rem 0.75rem;margin-right:0.5rem",c.textContent="Save";let l=document.createElement("button");l.className="discuss-action-btn",l.style.cssText="font-size:0.8125rem",l.textContent="Cancel",e.innerHTML="",e.appendChild(c),e.appendChild(l),r.focus(),r.setSelectionRange(r.value.length,r.value.length),c.addEventListener("click",()=>{this._saveEdit(t,r.value,e,a,n)}),l.addEventListener("click",()=>{i.innerHTML=a,e.innerHTML=n,this.container.querySelectorAll(`.discuss-edit-btn[data-id="${t}"]`).forEach(p=>{p.addEventListener("click",()=>this._startInlineEdit(t))})})}async _saveEdit(t,s,i,e,a){let n=this.editTokens.get(t);if(!n||!s.trim())return;let o=i.querySelector("button");o&&(o.disabled=!0,o.textContent="Saving\u2026");try{let r=await fetch(`${this.serverUrl}/api/comments/${t}`,{method:"PATCH",headers:{"Content-Type":"application/json","X-Edit-Token":n},credentials:"include",body:JSON.stringify({content:s.trim()})});if(r.ok)this._rawContent.set(t,s.trim()),this.init();else{let c=await r.json();alert(c.error||"Failed to save edit."),o&&(o.disabled=!1,o.textContent="Save")}}catch(r){console.error("[Discuss]",r),alert("Network error. Please try again."),o&&(o.disabled=!1,o.textContent="Save")}}renderComment(t,s,i,e){s=s||0;let a=t.is_pinned?'<span class="discuss-badge discuss-badge-info" style="margin-left:0.375rem">Pinned</span>':"",n=t.is_author?'<span class="discuss-badge discuss-badge-success" style="margin-left:0.375rem">Author</span>':"",o=t.edited_at?'<span style="margin-left:0.375rem;font-size:0.6875rem;color:var(--text-subtle);font-style:italic">(edited)</span>':"",r=new Date(t.created_at).toLocaleDateString(void 0,{year:"numeric",month:"short",day:"numeric"}),c=this.getAdminBadges(t),l=this.getAdminTooltip(t),p=this.getAdminControls(t),f=i?`<a href="#comment-${e}" class="discuss-reply-tag"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m15 10 5 5-5 5"/><path d="M4 4v7a4 4 0 0 0 4 4h12"/></svg>${i}</a>`:"",y=`
        ...
        `,m="";if(t.children.length>0)if(s>=3){let g=t.children.map(v=>this.renderComment(v,3,t.name,t.id)).join("");m=`
                ...
                `;let u=t.content;f&&(u.startsWith("<p>")?u=u.replace("<p>",`<p>${f}`):u=f+u);let $=t.children.length>0&&s<3?`
        ...
        `;return s<3?b:b+m}renderForm(t){let s=this.config.honeypot_question?`<input type="text" name="honeypot_answer_given" placeholder="${this.config.honeypot_question}" style="display:none" tabindex="-1" autocomplete="off">`:"";return`
