...
`;function M(){if(document.getElementById("discuss-styles"))return;let d=document.createElement("style");d.id="discuss-styles",d.textContent=S,document.head.appendChild(d)}function E(d){let s=parseInt(d.replace("#",""),16);return[s>>16&255,s>>8&255,s&255]}function A(d,s,e){return"#"+[d,s,e].map(o=>Math.round(Math.max(0,Math.min(255,o))).toString(16).padStart(2,"0")).join("")}function z(d,s,e){d/=255,s/=255,e/=255;let o=Math.max(d,s,e),t=Math.min(d,s,e),n,i,a=(o+t)/2;if(o===t)n=i=0;else{let r=o-t;switch(i=a>.5?r/(2-o-t):r/(o+t),o){case d:n=((s-e)/r+(s<e?6:0))/6;break;case s:n=((e-d)/r+2)/6;break;default:n=((d-s)/r+4)/6}}return[n*360,i,a]}function B(d,s,e){d/=360;let o=(i,a,r)=>(r<0&&(r+=1),r>1&&(r-=1),r<1/6?i+(a-i)*6*r:r<1/2?a:r<2/3?i+(a-i)*(2/3-r)*6:i);if(s===0){let i=Math.round(e*255);return[i,i,i]}let t=e<.5?e*(1+s):e+s-e*s,n=2*e-t;return[o(n,t,d+1/3),o(n,t,d),o(n,t,d-1/3)].map(i=>Math.round(i*255))}function j(d){let[s,e,o]=E(d),[t,n]=z(s,e,o),i={50:.96,100:.93,200:.86,300:.74,400:.6,500:.48,600:.38,700:.3,800:.22,900:.15},a={};for(let[r,c]of Object.entries(i)){let[l,m,b]=B(t,Math.min(n,.85),c);a[r]=A(l,m,b)}return a}var h=class{constructor(s){if(window.DiscussWidgetInstance=this,this.container=s.container,!this.container)return;let e=document.querySelector('link[rel="canonical"]')?.getAttribute("href");this.postUrl=s.postUrl||this.container.dataset.url||e||window.location.pathname,this.serverUrl=s.serverUrl||C,this.fetchUrl=s.fetchUrl||`${this.serverUrl}/api/comments?post_url=${encodeURIComponent(this.postUrl)}`,this.config={},this.primaryColor=s.primaryColor||null,this.init=this.init.bind(this),this.render=this.render.bind(this),this.renderComment=this.renderComment.bind(this),this.renderForm=this.renderForm.bind(this),this.handleSubmit=this.handleSubmit.bind(this),this.primaryColor&&this.applyTheme(this.primaryColor),this.init()}applyTheme(s){if(!s||!/^#[0-9a-fA-F]{6}$/.test(s))return;let e=j(s),o=this.container;o.style.setProperty("--b50",e[50]),o.style.setProperty("--b100",e[100]),o.style.setProperty("--b200",e[200]),o.style.setProperty("--b300",e[300]),o.style.setProperty("--b400",e[400]),o.style.setProperty("--b500",e[500]),o.style.setProperty("--b600",e[600]),o.style.setProperty("--b700",e[700]),o.style.setProperty("--b800",e[800]),o.style.setProperty("--b900",e[900]),o.style.setProperty("--accent-fg",e[700]),o.style.setProperty("--accent-surface",e[50]),o.style.setProperty("--focus-ring",e[700])}async init(){M(),this.container.innerHTML='<div style="padding:1rem;color:#64748b;font-family:sans-serif">Loading comments\u2026</div>';try{let s=await fetch(`${this.serverUrl}/api/comments/config`);s.ok&&(this.config=await s.json(),this.config.primary_color&&this.applyTheme(this.config.primary_color));let e=await fetch(this.fetchUrl);if(!e.ok)throw new Error("Failed to load comments");let o=await e.json();this.render(o)}catch(s){this.container.innerHTML='<div style="padding:1rem;color:#dc2626;font-family:sans-serif">Error loading comments.</div>',console.error("[Discuss]",s)}}buildTree(s){let e={},o=[];return s.forEach(t=>{t.children=[],e[t.id]=t}),s.forEach(t=>{t.parent_id===0||!e[t.parent_id]?o.push(t):e[t.parent_id].children.push(t)}),o}render(s){let e=this.buildTree(s);this.container.innerHTML=`
        ...
        `,this.container.querySelectorAll("form[data-parent]").forEach(t=>{t.addEventListener("submit",this.handleSubmit)}),this.container.querySelectorAll(".discuss-reply-tag").forEach(t=>{t.addEventListener("click",n=>{n.preventDefault();let i=n.currentTarget.getAttribute("href"),a=i.startsWith("#")?i.substring(1):i,r=document.getElementById(a);if(r){r.scrollIntoView({behavior:"smooth",block:"center"});let c=r.style.backgroundColor,l=document.documentElement.classList.contains("dark")||document.body.classList.contains("dark");r.style.backgroundColor=l?"#1e293b":"var(--b50)",r.style.borderRadius="8px",setTimeout(()=>{r.style.transition="background-color 500ms ease",r.style.backgroundColor=c,setTimeout(()=>{r.style.transition="",r.style.borderRadius=""},500)},1500)}})}),this.container.querySelectorAll(".discuss-reply-btn").forEach(t=>{t.addEventListener("click",n=>{let i=n.currentTarget.dataset.id,a=document.getElementById(`discuss-reply-form-${i}`);a&&a.classList.toggle("discuss-hidden")})});let o=t=>{let n=t.currentTarget.dataset.id,i=!1,a=document.getElementById(`discuss-collapse-target-${n}`);a&&(i=a.classList.toggle("discuss-hidden"));let r=document.getElementById(`discuss-children-${n}`);r&&(a?r.classList.toggle("discuss-hidden",i):i=r.classList.toggle("discuss-hidden"));let c=document.querySelector(`.discuss-collapse-btn[data-id="${n}"]`);if(c){let l=c.querySelector("svg");l&&(l.style.transform=i?"rotate(-90deg)":"rotate(0deg)")}};this.container.querySelectorAll(".discuss-collapse-btn, .discuss-collapse-line").forEach(t=>{t.addEventListener("click",o)}),this.container.querySelectorAll(".discuss-share-btn").forEach(t=>{t.addEventListener("click",async n=>{let i=n.currentTarget,a=i.dataset.id,r=`${window.location.origin}${window.location.pathname}#comment-${a}`;if(navigator.share){try{await navigator.share({title:document.title,url:r})}catch(c){if(c.name==="AbortError")return;await this.copyToClipboard(i,r)}return}await this.copyToClipboard(i,r)})})}async copyToClipboard(s,e){try{if(navigator.clipboard)await navigator.clipboard.writeText(e);else{window.prompt("Copy link:",e);return}s.innerHTML=`${u.share} <span>Copied!</span>`,setTimeout(()=>{s.innerHTML=`${u.share} <span>Share</span>`},2e3)}catch{window.prompt("Copy link:",e)}}getInitialsColor(s){let e=["#0d4891","#16a34a","#b45309","#1e40af","#dc2626","#6b21a8","#be185d","#0369a1"],o=0;for(let t=0;t<s.length;t++)o=s.charCodeAt(t)+((o<<5)-o);return e[Math.abs(o)%e.length]}getAvatarHtml(s){let e=s.name?s.name.charAt(0).toUpperCase():"U";return`
