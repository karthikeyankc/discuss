...
`;function M(){if(document.getElementById("discuss-styles"))return;let d=document.createElement("style");d.id="discuss-styles",d.textContent=S,document.head.appendChild(d)}function E(d){let s=parseInt(d.replace("#",""),16);return[s>>16&255,s>>8&255,s&255]}function z(d,s,e){return"#"+[d,s,e].map(o=>Math.round(Math.max(0,Math.min(255,o))).toString(16).padStart(2,"0")).join("")}function A(d,s,e){d/=255,s/=255,e/=255;let o=Math.max(d,s,e),t=Math.min(d,s,e),i,n,a=(o+t)/2;if(o===t)i=n=0;else{let r=o-t;switch(n=a>.5?r/(2-o-t):r/(o+t),o){case d:i=((s-e)/r+(s<e?6:0))/6;break;case s:i=((e-d)/r+2)/6;break;default:i=((d-s)/r+4)/6}}return[i*360,n,a]}function I(d,s,e){d/=360;let o=(n,a,r)=>(r<0&&(r+=1),r>1&&(r-=1),r<1/6?n+(a-n)*6*r:r<1/2?a:r<2/3?n+(a-n)*(2/3-r)*6:n);if(s===0){let n=Math.round(e*255);return[n,n,n]}let t=e<.5?e*(1+s):e+s-e*s,i=2*e-t;return[o(i,t,d+1/3),o(i,t,d),o(i,t,d-1/3)].map(n=>Math.round(n*255))}function B(d){let[s,e,o]=E(d),[t,i]=A(s,e,o),n={50:.96,100:.93,200:.86,300:.74,400:.6,500:.48,600:.38,700:.3,800:.22,900:.15},a={};for(let[r,c]of Object.entries(n)){let[l,m,b]=I(t,Math.min(i,.85),c);a[r]=z(l,m,b)}return a}var h=class{constructor(s){window.DiscussWidgetInstance=this,this.container=s.container,this.container&&(this.postUrl=s.postUrl||this.container.dataset.url||window.location.pathname,this.serverUrl=s.serverUrl||C,this.fetchUrl=s.fetchUrl||`${this.serverUrl}/api/comments?post_url=${encodeURIComponent(this.postUrl)}`,this.config={},this.primaryColor=s.primaryColor||null,this.domainId=s.domainId||null,this.init=this.init.bind(this),this.render=this.render.bind(this),this.renderComment=this.renderComment.bind(this),this.renderForm=this.renderForm.bind(this),this.handleSubmit=this.handleSubmit.bind(this),this.primaryColor&&this.applyTheme(this.primaryColor),this.init())}applyTheme(s){if(!s||!/^#[0-9a-fA-F]{6}$/.test(s))return;let e=B(s),o=this.container;o.style.setProperty("--b50",e[50]),o.style.setProperty("--b100",e[100]),o.style.setProperty("--b200",e[200]),o.style.setProperty("--b300",e[300]),o.style.setProperty("--b400",e[400]),o.style.setProperty("--b500",e[500]),o.style.setProperty("--b600",e[600]),o.style.setProperty("--b700",e[700]),o.style.setProperty("--b800",e[800]),o.style.setProperty("--b900",e[900]),o.style.setProperty("--accent-fg",e[700]),o.style.setProperty("--accent-surface",e[50]),o.style.setProperty("--focus-ring",e[700])}async init(){M(),this.container.innerHTML='<div style="padding:1rem;color:#64748b;font-family:inherit">Loading comments\u2026</div>';try{let s=await fetch(`${this.serverUrl}/api/comments/config`);s.ok&&(this.config=await s.json(),this.config.primary_color&&this.applyTheme(this.config.primary_color));let e=await fetch(this.fetchUrl);if(!e.ok)throw new Error("Failed to load comments");let o=await e.json();this.render(o)}catch(s){this.container.innerHTML='<div style="padding:1rem;color:#dc2626;font-family:inherit">Error loading comments.</div>',console.error("[Discuss]",s)}}buildTree(s){let e={},o=[];return s.forEach(t=>{t.children=[],e[t.id]=t}),s.forEach(t=>{t.parent_id===0||!e[t.parent_id]?o.push(t):e[t.parent_id].children.push(t)}),o}render(s){let e=this.buildTree(s);this.container.innerHTML=`
        ...
        `,this.container.querySelectorAll("form[data-parent]").forEach(t=>{t.addEventListener("submit",this.handleSubmit)}),this.container.querySelectorAll(".discuss-reply-tag").forEach(t=>{t.addEventListener("click",i=>{i.preventDefault();let n=i.currentTarget.getAttribute("href"),a=n.startsWith("#")?n.substring(1):n,r=document.getElementById(a);if(r){r.scrollIntoView({behavior:"smooth",block:"center"});let c=r.style.backgroundColor,l=document.documentElement.classList.contains("dark")||document.body.classList.contains("dark");r.style.backgroundColor=l?"#1e293b":"var(--b50)",r.style.borderRadius="8px",setTimeout(()=>{r.style.transition="background-color 500ms ease",r.style.backgroundColor=c,setTimeout(()=>{r.style.transition="",r.style.borderRadius=""},500)},1500)}})}),this.container.querySelectorAll(".discuss-reply-btn").forEach(t=>{t.addEventListener("click",i=>{let n=i.currentTarget.dataset.id,a=document.getElementById(`discuss-reply-form-${n}`);a&&a.classList.toggle("discuss-hidden")})});let o=t=>{let i=t.currentTarget.dataset.id,n=!1,a=document.getElementById(`discuss-collapse-target-${i}`);a&&(n=a.classList.toggle("discuss-hidden"));let r=document.getElementById(`discuss-children-${i}`);r&&(a?r.classList.toggle("discuss-hidden",n):n=r.classList.toggle("discuss-hidden"));let c=document.querySelector(`.discuss-collapse-btn[data-id="${i}"]`);if(c){let l=c.querySelector("svg");l&&(l.style.transform=n?"rotate(-90deg)":"rotate(0deg)")}};this.container.querySelectorAll(".discuss-collapse-btn, .discuss-collapse-line").forEach(t=>{t.addEventListener("click",o)}),this.container.querySelectorAll(".discuss-share-btn").forEach(t=>{t.addEventListener("click",async i=>{let n=i.currentTarget,a=n.dataset.id,r=`${window.location.origin}${window.location.pathname}#comment-${a}`;if(navigator.share){try{await navigator.share({title:document.title,url:r})}catch(c){if(c.name==="AbortError")return;await this.copyToClipboard(n,r)}return}await this.copyToClipboard(n,r)})})}async copyToClipboard(s,e){try{if(navigator.clipboard)await navigator.clipboard.writeText(e);else{window.prompt("Copy link:",e);return}s.innerHTML=`${u.share} <span>Copied!</span>`,setTimeout(()=>{s.innerHTML=`${u.share} <span>Share</span>`},2e3)}catch{window.prompt("Copy link:",e)}}getInitialsColor(s){let e=["#0d4891","#16a34a","#b45309","#1e40af","#dc2626","#6b21a8","#be185d","#0369a1"],o=0;for(let t=0;t<s.length;t++)o=s.charCodeAt(t)+((o<<5)-o);return e[Math.abs(o)%e.length]}getAvatarHtml(s){let e=s.name?s.name.charAt(0).toUpperCase():"U";return`
        ...
        `}toggleEditForm(s){let e=document.getElementById(`discuss-edit-form-${s}`);e&&(e.style.display=e.style.display==="none"?"block":"none")}async saveEdit(s){let e=document.getElementById(`discuss-edit-name-${s}`)?.value.trim(),o=document.getElementById(`discuss-edit-email-${s}`)?.value.trim(),t=document.getElementById(`discuss-edit-content-${s}`)?.value,i=document.getElementById(`discuss-edit-date-${s}`)?.value,n=i?new Date(i).getTime():null;if(!e||!t){window.app?.showToast("Name and content are required.","error");return}try{let a=await fetch(`/api/admin/comments/${s}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e,email:o,content:t,created_at:n})});if(!a.ok){let m=await a.json();window.app?.showToast(m.error||"Failed to save.","error");return}let{content:r}=await a.json(),c=document.querySelector(`#comment-${s} .discuss-comment-body`);c&&(c.innerHTML=r);let l=document.querySelector(`#comment-${s} span[style*="font-weight:600"]`);if(l&&(l.textContent=e),i){let m=document.querySelector(`#comment-${s} span[style*="color:var(--t4)"]`);m&&(m.textContent=new Date(i).toLocaleDateString(void 0,{year:"numeric",month:"short",day:"numeric"}))}this.toggleEditForm(s),window.app?.showToast("Comment updated.")}catch{window.app?.showToast("Network error.","error")}}async toggleApprove(s,e){try{if((await fetch(`/api/admin/comments/${s}/approve`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({is_approved:e?0:1})})).ok){let t=e?0:1,i=document.querySelector(`.discuss-action-btn[onclick*="toggleApprove(${s}"]`);i&&(i.setAttribute("onclick",`window.DiscussWidgetInstance.toggleApprove(${s}, ${t})`),t?(i.style.color="",i.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg> <span>Unapprove</span>',document.querySelector(`#comment-${s} .discuss-badge-warning`)?.remove()):(i.style.color="#16a34a",i.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg> <span>Approve</span>',document.querySelector(`#comment-${s} span[style*="font-weight:600"]`)?.insertAdjacentHTML("afterend",'<span class="discuss-badge discuss-badge-warning" style="margin-left:0.375rem">Pending</span>'))),window.app?.refreshPendingBadge()}else window.app?.showToast("Failed to update status.","error")}catch{window.app?.showToast("Network error.","error")}}async togglePin(s,e){try{if((await fetch(`/api/admin/comments/${s}/pin`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({is_pinned:e?0:1})})).ok){let t=e?0:1,i=document.querySelector(`.discuss-action-btn[onclick*="togglePin(${s}"]`);i&&(i.setAttribute("onclick",`window.DiscussWidgetInstance.togglePin(${s}, ${t})`),t?(i.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 17v5"/><path d="M9 10.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V16a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V7a1 1 0 0 1 1-1 2 2 0 0 0 0-4H8a2 2 0 0 0 0 4 1 1 0 0 1 1 1z"/><line x1="2" x2="22" y1="2" y2="22"/></svg> <span>Unpin</span>',document.querySelector(`#comment-${s} span[style*="font-weight:600"]`)?.insertAdjacentHTML("afterend",'<span class="discuss-badge discuss-badge-info" style="margin-left:0.375rem">Pinned</span>')):(i.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 17v5"/><path d="M9 10.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V16a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V7a1 1 0 0 1 1-1 2 2 0 0 0 0-4H8a2 2 0 0 0 0 4 1 1 0 0 1 1 1z"/></svg> <span>Pin</span>',document.querySelector(`#comment-${s} .discuss-badge-info`)?.remove()))}else window.app?.showToast("Failed to update pin.","error")}catch{window.app?.showToast("Network error.","error")}}async deleteComment(s){if(await window.app?.showConfirm({title:"Delete comment?",message:"This will permanently remove the comment. This cannot be undone.",confirmLabel:"Delete",isDanger:!0}))try{if((await fetch(`/api/admin/comments/${s}`,{method:"DELETE"})).ok){let t=document.getElementById(`comment-${s}`);t&&(t.style.transition="opacity 300ms ease, transform 300ms ease",t.style.opacity="0",t.style.transform="translateY(-8px)",setTimeout(()=>t.remove(),300)),window.app?.refreshPendingBadge()}else window.app?.showToast("Failed to delete comment.","error")}catch{window.app?.showToast("Network error.","error")}}};window.DiscussWidget=y;})();
