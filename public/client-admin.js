...
`;function M(){if(document.getElementById("discuss-styles"))return;let c=document.createElement("style");c.id="discuss-styles",c.textContent=S,document.head.appendChild(c)}function E(c){let e=parseInt(c.replace("#",""),16);return[e>>16&255,e>>8&255,e&255]}function z(c,e,s){return"#"+[c,e,s].map(o=>Math.round(Math.max(0,Math.min(255,o))).toString(16).padStart(2,"0")).join("")}function A(c,e,s){c/=255,e/=255,s/=255;let o=Math.max(c,e,s),t=Math.min(c,e,s),i,n,a=(o+t)/2;if(o===t)i=n=0;else{let r=o-t;switch(n=a>.5?r/(2-o-t):r/(o+t),o){case c:i=((e-s)/r+(e<s?6:0))/6;break;case e:i=((s-c)/r+2)/6;break;default:i=((c-e)/r+4)/6}}return[i*360,n,a]}function I(c,e,s){c/=360;let o=(n,a,r)=>(r<0&&(r+=1),r>1&&(r-=1),r<1/6?n+(a-n)*6*r:r<1/2?a:r<2/3?n+(a-n)*(2/3-r)*6:n);if(e===0){let n=Math.round(s*255);return[n,n,n]}let t=s<.5?s*(1+e):s+e-s*e,i=2*s-t;return[o(i,t,c+1/3),o(i,t,c),o(i,t,c-1/3)].map(n=>Math.round(n*255))}function B(c){let[e,s,o]=E(c),[t,i]=A(e,s,o),n={50:.96,100:.93,200:.86,300:.74,400:.6,500:.48,600:.38,700:.3,800:.22,900:.15},a={};for(let[r,d]of Object.entries(n)){let[l,m,b]=I(t,Math.min(i,.85),d);a[r]=z(l,m,b)}return a}var h=class{constructor(e){window.DiscussWidgetInstance=this,this.container=e.container,this.container&&(this.postUrl=e.postUrl||this.container.dataset.url||window.location.pathname,this.serverUrl=e.serverUrl||C,this.fetchUrl=e.fetchUrl||`${this.serverUrl}/api/comments?post_url=${encodeURIComponent(this.postUrl)}`,this.config={},this.primaryColor=e.primaryColor||null,this.domainId=e.domainId||null,this.init=this.init.bind(this),this.render=this.render.bind(this),this.renderComment=this.renderComment.bind(this),this.renderForm=this.renderForm.bind(this),this.handleSubmit=this.handleSubmit.bind(this),this.primaryColor&&this.applyTheme(this.primaryColor),this.init())}applyTheme(e){if(!e||!/^#[0-9a-fA-F]{6}$/.test(e))return;let s=B(e),o=this.container;o.style.setProperty("--b50",s[50]),o.style.setProperty("--b100",s[100]),o.style.setProperty("--b200",s[200]),o.style.setProperty("--b300",s[300]),o.style.setProperty("--b400",s[400]),o.style.setProperty("--b500",s[500]),o.style.setProperty("--b600",s[600]),o.style.setProperty("--b700",s[700]),o.style.setProperty("--b800",s[800]),o.style.setProperty("--b900",s[900]),o.style.setProperty("--accent-fg",s[700]),o.style.setProperty("--accent-surface",s[50]),o.style.setProperty("--focus-ring",s[700])}async init(){M(),this.container.innerHTML='<div style="padding:1rem;color:#64748b;font-family:sans-serif">Loading comments\u2026</div>';try{let e=await fetch(`${this.serverUrl}/api/comments/config`);e.ok&&(this.config=await e.json(),this.config.primary_color&&this.applyTheme(this.config.primary_color));let s=await fetch(this.fetchUrl);if(!s.ok)throw new Error("Failed to load comments");let o=await s.json();this.render(o)}catch(e){this.container.innerHTML='<div style="padding:1rem;color:#dc2626;font-family:sans-serif">Error loading comments.</div>',console.error("[Discuss]",e)}}buildTree(e){let s={},o=[];return e.forEach(t=>{t.children=[],s[t.id]=t}),e.forEach(t=>{t.parent_id===0||!s[t.parent_id]?o.push(t):s[t.parent_id].children.push(t)}),o}render(e){let s=this.buildTree(e);this.container.innerHTML=`
            <div class="discuss-font-sans" style="color:var(--t1)">
                ...
                ${s.length>0?`
                <div>
                    <h4 class="discuss-text-sm discuss-font-semibold discuss-uppercase discuss-tracking-wide" style="margin:0 0 1.25rem;color:var(--t4)">${s.length} Comment${s.length!==1?"s":""}</h4>
                    <div class="discuss-flex discuss-flex-col discuss-gap-6">
                        ${s.map(t=>this.renderComment(t)).join("")}
                    </div>
                </div>`:""}
            </div>
        `,this.container.querySelectorAll("form[data-parent]").forEach(t=>{t.addEventListener("submit",this.handleSubmit)}),this.container.querySelectorAll(".discuss-reply-tag").forEach(t=>{t.addEventListener("click",i=>{i.preventDefault();let n=i.currentTarget.getAttribute("href"),a=n.startsWith("#")?n.substring(1):n,r=document.getElementById(a);if(r){r.scrollIntoView({behavior:"smooth",block:"center"});let d=r.style.backgroundColor,l=document.documentElement.classList.contains("dark")||document.body.classList.contains("dark");r.style.backgroundColor=l?"#1e293b":"var(--b50)",r.style.borderRadius="8px",setTimeout(()=>{r.style.transition="background-color 500ms ease",r.style.backgroundColor=d,setTimeout(()=>{r.style.transition="",r.style.borderRadius=""},500)},1500)}})}),this.container.querySelectorAll(".discuss-reply-btn").forEach(t=>{t.addEventListener("click",i=>{let n=i.currentTarget.dataset.id,a=document.getElementById(`discuss-reply-form-${n}`);a&&a.classList.toggle("discuss-hidden")})});let o=t=>{let i=t.currentTarget.dataset.id,n=!1,a=document.getElementById(`discuss-collapse-target-${i}`);a&&(n=a.classList.toggle("discuss-hidden"));let r=document.getElementById(`discuss-children-${i}`);r&&(a?r.classList.toggle("discuss-hidden",n):n=r.classList.toggle("discuss-hidden"));let d=document.querySelector(`.discuss-collapse-btn[data-id="${i}"]`);if(d){let l=d.querySelector("svg");l&&(l.style.transform=n?"rotate(-90deg)":"rotate(0deg)")}};this.container.querySelectorAll(".discuss-collapse-btn, .discuss-collapse-line").forEach(t=>{t.addEventListener("click",o)}),this.container.querySelectorAll(".discuss-share-btn").forEach(t=>{t.addEventListener("click",async i=>{let n=i.currentTarget,a=n.dataset.id,r=`${window.location.origin}${window.location.pathname}#comment-${a}`;if(navigator.share){try{await navigator.share({title:document.title,url:r})}catch(d){if(d.name==="AbortError")return;await this.copyToClipboard(n,r)}return}await this.copyToClipboard(n,r)})})}async copyToClipboard(e,s){try{if(navigator.clipboard)await navigator.clipboard.writeText(s);else{window.prompt("Copy link:",s);return}e.innerHTML=`${u.share} <span>Copied!</span>`,setTimeout(()=>{e.innerHTML=`${u.share} <span>Share</span>`},2e3)}catch{window.prompt("Copy link:",s)}}getInitialsColor(e){let s=["#0d4891","#16a34a","#b45309","#1e40af","#dc2626","#6b21a8","#be185d","#0369a1"],o=0;for(let t=0;t<e.length;t++)o=e.charCodeAt(t)+((o<<5)-o);return s[Math.abs(o)%s.length]}getAvatarHtml(e){let s=e.name?e.name.charAt(0).toUpperCase():"U";return`
        ...
        `,g="";if(e.children.length>0)if(s>=3){let f=e.children.map($=>this.renderComment($,3,e.name,e.id)).join("");g=`
                ...
                `;let p=e.content;m&&(p.startsWith("<p>")?p=p.replace("<p>",`<p>${m}`):p=m+p);let k=e.children.length>0&&s<3?`
        ...
        `:""}getAdminControls(e){let s=(e.content_raw||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),o=new Date(e.created_at).toISOString().slice(0,16);return`
            ...
            <div id="discuss-edit-form-${e.id}"
                ...
                <div style="margin-bottom:0.75rem">
                    ...
                    <textarea id="discuss-edit-content-${e.id}" rows="5"
                              style="width:100%;padding:0.5rem 0.625rem;border:1px solid var(--bd-control);border-radius:6px;font-size:0.875rem;font-family:monospace;color:var(--t1);background:var(--s1);resize:vertical">${s}</textarea>
                </div>
