...
`;function M(){if(document.getElementById("discuss-styles"))return;let c=document.createElement("style");c.id="discuss-styles",c.textContent=S,document.head.appendChild(c)}function E(c){let s=parseInt(c.replace("#",""),16);return[s>>16&255,s>>8&255,s&255]}function A(c,s,e){return"#"+[c,s,e].map(o=>Math.round(Math.max(0,Math.min(255,o))).toString(16).padStart(2,"0")).join("")}function z(c,s,e){c/=255,s/=255,e/=255;let o=Math.max(c,s,e),t=Math.min(c,s,e),n,i,a=(o+t)/2;if(o===t)n=i=0;else{let r=o-t;switch(i=a>.5?r/(2-o-t):r/(o+t),o){case c:n=((s-e)/r+(s<e?6:0))/6;break;case s:n=((e-c)/r+2)/6;break;default:n=((c-s)/r+4)/6}}return[n*360,i,a]}function B(c,s,e){c/=360;let o=(i,a,r)=>(r<0&&(r+=1),r>1&&(r-=1),r<1/6?i+(a-i)*6*r:r<1/2?a:r<2/3?i+(a-i)*(2/3-r)*6:i);if(s===0){let i=Math.round(e*255);return[i,i,i]}let t=e<.5?e*(1+s):e+s-e*s,n=2*e-t;return[o(n,t,c+1/3),o(n,t,c),o(n,t,c-1/3)].map(i=>Math.round(i*255))}function j(c){let[s,e,o]=E(c),[t,n]=z(s,e,o),i={50:.96,100:.93,200:.86,300:.74,400:.6,500:.48,600:.38,700:.3,800:.22,900:.15},a={};for(let[r,d]of Object.entries(i)){let[l,m,b]=B(t,Math.min(n,.85),d);a[r]=A(l,m,b)}return a}var h=class{constructor(s){if(window.DiscussWidgetInstance=this,this.container=s.container,!this.container)return;let e=document.querySelector('link[rel="canonical"]')?.getAttribute("href"),o=e?new URL(e,window.location.href).pathname:null;this.postUrl=s.postUrl||this.container.dataset.url||o||window.location.pathname,this.serverUrl=s.serverUrl||C,this.fetchUrl=s.fetchUrl||`${this.serverUrl}/api/comments?post_url=${encodeURIComponent(this.postUrl)}`,this.config={},this.primaryColor=s.primaryColor||null,this.init=this.init.bind(this),this.render=this.render.bind(this),this.renderComment=this.renderComment.bind(this),this.renderForm=this.renderForm.bind(this),this.handleSubmit=this.handleSubmit.bind(this),this.primaryColor&&this.applyTheme(this.primaryColor),this.init()}applyTheme(s){if(!s||!/^#[0-9a-fA-F]{6}$/.test(s))return;let e=j(s),o=this.container;o.style.setProperty("--b50",e[50]),o.style.setProperty("--b100",e[100]),o.style.setProperty("--b200",e[200]),o.style.setProperty("--b300",e[300]),o.style.setProperty("--b400",e[400]),o.style.setProperty("--b500",e[500]),o.style.setProperty("--b600",e[600]),o.style.setProperty("--b700",e[700]),o.style.setProperty("--b800",e[800]),o.style.setProperty("--b900",e[900]),o.style.setProperty("--accent-fg",e[700]),o.style.setProperty("--accent-surface",e[50]),o.style.setProperty("--focus-ring",e[700])}async init(){M(),this.container.innerHTML='<div style="padding:1rem;color:#64748b;font-family:sans-serif">Loading comments\u2026</div>';try{let s=await fetch(`${this.serverUrl}/api/comments/config`);s.ok&&(this.config=await s.json(),this.config.primary_color&&this.applyTheme(this.config.primary_color));let e=await fetch(this.fetchUrl);if(!e.ok)throw new Error("Failed to load comments");let o=await e.json();this.render(o)}catch(s){this.container.innerHTML='<div style="padding:1rem;color:#dc2626;font-family:sans-serif">Error loading comments.</div>',console.error("[Discuss]",s)}}buildTree(s){let e={},o=[];return s.forEach(t=>{t.children=[],e[t.id]=t}),s.forEach(t=>{t.parent_id===0||!e[t.parent_id]?o.push(t):e[t.parent_id].children.push(t)}),o}render(s){let e=this.buildTree(s);this.container.innerHTML=`
        ...
        `,this.container.querySelectorAll("form[data-parent]").forEach(t=>{t.addEventListener("submit",this.handleSubmit)}),this.container.querySelectorAll(".discuss-reply-tag").forEach(t=>{t.addEventListener("click",n=>{n.preventDefault();let i=n.currentTarget.getAttribute("href"),a=i.startsWith("#")?i.substring(1):i,r=document.getElementById(a);if(r){r.scrollIntoView({behavior:"smooth",block:"center"});let d=r.style.backgroundColor,l=document.documentElement.classList.contains("dark")||document.body.classList.contains("dark");r.style.backgroundColor=l?"#1e293b":"var(--b50)",r.style.borderRadius="8px",setTimeout(()=>{r.style.transition="background-color 500ms ease",r.style.backgroundColor=d,setTimeout(()=>{r.style.transition="",r.style.borderRadius=""},500)},1500)}})}),this.container.querySelectorAll(".discuss-reply-btn").forEach(t=>{t.addEventListener("click",n=>{let i=n.currentTarget.dataset.id,a=document.getElementById(`discuss-reply-form-${i}`);a&&a.classList.toggle("discuss-hidden")})});let o=t=>{let n=t.currentTarget.dataset.id,i=!1,a=document.getElementById(`discuss-collapse-target-${n}`);a&&(i=a.classList.toggle("discuss-hidden"));let r=document.getElementById(`discuss-children-${n}`);r&&(a?r.classList.toggle("discuss-hidden",i):i=r.classList.toggle("discuss-hidden"));let d=document.querySelector(`.discuss-collapse-btn[data-id="${n}"]`);if(d){let l=d.querySelector("svg");l&&(l.style.transform=i?"rotate(-90deg)":"rotate(0deg)")}};this.container.querySelectorAll(".discuss-collapse-btn, .discuss-collapse-line").forEach(t=>{t.addEventListener("click",o)}),this.container.querySelectorAll(".discuss-share-btn").forEach(t=>{t.addEventListener("click",async n=>{let i=n.currentTarget,a=i.dataset.id,r=`${window.location.origin}${window.location.pathname}#comment-${a}`;if(navigator.share){try{await navigator.share({title:document.title,url:r})}catch(d){if(d.name==="AbortError")return;await this.copyToClipboard(i,r)}return}await this.copyToClipboard(i,r)})})}async copyToClipboard(s,e){try{if(navigator.clipboard)await navigator.clipboard.writeText(e);else{window.prompt("Copy link:",e);return}s.innerHTML=`${u.share} <span>Copied!</span>`,setTimeout(()=>{s.innerHTML=`${u.share} <span>Share</span>`},2e3)}catch{window.prompt("Copy link:",e)}}getInitialsColor(s){let e=["#0d4891","#16a34a","#b45309","#1e40af","#dc2626","#6b21a8","#be185d","#0369a1"],o=0;for(let t=0;t<s.length;t++)o=s.charCodeAt(t)+((o<<5)-o);return e[Math.abs(o)%e.length]}getAvatarHtml(s){let e=s.name?s.name.charAt(0).toUpperCase():"U";return`
        ...
        `,g="";if(s.children.length>0)if(e>=3){let f=s.children.map($=>this.renderComment($,3,s.name,s.id)).join("");g=`
                ...
                `;let p=s.content;m&&(p.startsWith("<p>")?p=p.replace("<p>",`<p>${m}`):p=m+p);let k=s.children.length>0&&e<3?`
            ...
            <div class="discuss-flex discuss-comment-row" id="comment-${s.id}">
                ...
                <div class="discuss-comment-content" style="min-width:0">
                    ...
                    <div id="discuss-collapse-target-${s.id}">
                        <div class="discuss-comment-body">${p}</div>
        ...
        `;return e<3?w:w+g}renderForm(s){let e=this.config.honeypot_question?`<input type="text" name="honeypot_answer_given" placeholder="${this.config.honeypot_question}" style="display:none" tabindex="-1" autocomplete="off">`:"";return`
        ...
        `}async handleSubmit(s){s.preventDefault();let e=s.target,o=e.dataset.parent,t=e.querySelector('[type="submit"]');t.disabled=!0,t.innerHTML='<span class="discuss-spinner discuss-spinner-sm" style="margin-right:0.5rem"></span> Posting\u2026';let n={name:e.name.value.trim(),email:e.email.value.trim(),content:e.content.value.trim(),post_url:this.postUrl,parent_id:parseInt(o,10),honeypot_field:e.honeypot_field.value,honeypot_answer_given:e.honeypot_answer_given?e.honeypot_answer_given.value:void 0};try{let i=await fetch(`${this.serverUrl}/api/comments`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify(n)});if(i.ok){if(e.reset(),parseInt(o,10)!==0){let a=document.getElementById(`discuss-reply-form-${o}`);a&&a.classList.add("discuss-hidden")}this.init()}else{let a=await i.json();alert(a.error||"Failed to post comment.")}}catch(i){console.error("[Discuss]",i),alert("Network error. Please try again.")}finally{t.disabled=!1,t.textContent="Post Comment"}}};window.DiscussWidget=h;var v=document.getElementById("discuss-comments");v&&v.dataset.isAdmin!=="true"&&new h({container:v});var y=class extends h{constructor(s){super(s),this.isAdmin=!0}getAdminBadges(s){return s.is_approved?"":'<span class="discuss-badge discuss-badge-warning" style="margin-left:0.375rem">Pending</span>'}getAdminTooltip(s){return s.email?`
        ...
        `:""}getAdminControls(s){let e=(s.content_raw||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),o=new Date(s.created_at).toISOString().slice(0,16);return`
            <div style="width:1px;height:12px;background:var(--bds);margin:0 0.25rem"></div>
            <button class="discuss-action-btn" ${s.is_approved?"":'style="color:#16a34a"'}
                    onclick="window.DiscussWidgetInstance.toggleApprove(${s.id}, ${s.is_approved})">
                ${s.is_approved?'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg> <span>Unapprove</span>':'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg> <span>Approve</span>'}
            </button>
            <button class="discuss-action-btn" onclick="window.DiscussWidgetInstance.togglePin(${s.id}, ${s.is_pinned})">
            ...
            <button class="discuss-action-btn" onclick="window.DiscussWidgetInstance.toggleEditForm(${s.id})">
            ...
            <button class="discuss-action-btn" style="color:#ef4444"
                    onclick="window.DiscussWidgetInstance.deleteComment(${s.id})">
            ...
            <div id="discuss-edit-form-${s.id}"
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;margin-bottom:0.75rem">
                    <div>
                        ...
                        <input type="text" id="discuss-edit-name-${s.id}"
                    ...
                    <div>
                        ...
                        <input type="email" id="discuss-edit-email-${s.id}"
                               style="width:100%;padding:0.5rem 0.625rem;border:1px solid var(--bd-control);border-radius:6px;font-size:0.875rem;color:var(--t1);background:var(--s1)"
                               value="${(s.email||"").replace(/"/g,"&quot;")}">
                    </div>
                </div>
                <div style="margin-bottom:0.75rem">
                    ...
                    <input type="datetime-local" id="discuss-edit-date-${s.id}"
                           style="width:100%;padding:0.5rem 0.625rem;border:1px solid var(--bd-control);border-radius:6px;font-size:0.875rem;color:var(--t1);background:var(--s1)"
                           value="${o}">
                </div>
                <div style="margin-bottom:0.75rem">
                    <label style="display:block;font-size:0.75rem;font-weight:600;color:var(--t2);margin-bottom:0.25rem">Content (Markdown)</label>
                    <textarea id="discuss-edit-content-${s.id}" rows="5"
                              style="width:100%;padding:0.5rem 0.625rem;border:1px solid var(--bd-control);border-radius:6px;font-size:0.875rem;font-family:monospace;color:var(--t1);background:var(--s1);resize:vertical">${e}</textarea>
                </div>
                <div style="display:flex;gap:0.5rem">
                    <button class="discuss-action-btn" style="background:var(--b600);color:#fff;padding:0.375rem 0.75rem;border-radius:6px"
                            onclick="window.DiscussWidgetInstance.saveEdit(${s.id})">Save</button>
                    <button class="discuss-action-btn"
                            onclick="window.DiscussWidgetInstance.toggleEditForm(${s.id})">Cancel</button>
                </div>
            </div>
        `}toggleEditForm(s){let e=document.getElementById(`discuss-edit-form-${s}`);e&&(e.style.display=e.style.display==="none"?"block":"none")}async saveEdit(s){let e=document.getElementById(`discuss-edit-name-${s}`)?.value.trim(),o=document.getElementById(`discuss-edit-email-${s}`)?.value.trim(),t=document.getElementById(`discuss-edit-content-${s}`)?.value,n=document.getElementById(`discuss-edit-date-${s}`)?.value,i=n?new Date(n).getTime():null;if(!e||!t){window.app?.showToast("Name and content are required.","error");return}try{let a=await fetch(`/api/admin/comments/${s}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e,email:o,content:t,created_at:i})});if(!a.ok){let m=await a.json();window.app?.showToast(m.error||"Failed to save.","error");return}let{content:r}=await a.json(),d=document.querySelector(`#comment-${s} .discuss-comment-body`);d&&(d.innerHTML=r);let l=document.querySelector(`#comment-${s} span[style*="font-weight:600"]`);if(l&&(l.textContent=e),n){let m=document.querySelector(`#comment-${s} span[style*="color:var(--t4)"]`);m&&(m.textContent=new Date(n).toLocaleDateString(void 0,{year:"numeric",month:"short",day:"numeric"}))}this.toggleEditForm(s),window.app?.showToast("Comment updated.")}catch{window.app?.showToast("Network error.","error")}}async toggleApprove(s,e){try{if((await fetch(`/api/admin/comments/${s}/approve`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({is_approved:e?0:1})})).ok){let t=e?0:1,n=document.querySelector(`.discuss-action-btn[onclick*="toggleApprove(${s}"]`);n&&(n.setAttribute("onclick",`window.DiscussWidgetInstance.toggleApprove(${s}, ${t})`),t?(n.style.color="",n.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg> <span>Unapprove</span>',document.querySelector(`#comment-${s} .discuss-badge-warning`)?.remove()):(n.style.color="#16a34a",n.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg> <span>Approve</span>',document.querySelector(`#comment-${s} span[style*="font-weight:600"]`)?.insertAdjacentHTML("afterend",'<span class="discuss-badge discuss-badge-warning" style="margin-left:0.375rem">Pending</span>'))),window.app?.refreshPendingBadge()}else window.app?.showToast("Failed to update status.","error")}catch{window.app?.showToast("Network error.","error")}}async togglePin(s,e){try{if((await fetch(`/api/admin/comments/${s}/pin`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({is_pinned:e?0:1})})).ok){let t=e?0:1,n=document.querySelector(`.discuss-action-btn[onclick*="togglePin(${s}"]`);n&&(n.setAttribute("onclick",`window.DiscussWidgetInstance.togglePin(${s}, ${t})`),t?(n.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 17v5"/><path d="M9 10.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V16a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V7a1 1 0 0 1 1-1 2 2 0 0 0 0-4H8a2 2 0 0 0 0 4 1 1 0 0 1 1 1z"/><line x1="2" x2="22" y1="2" y2="22"/></svg> <span>Unpin</span>',document.querySelector(`#comment-${s} span[style*="font-weight:600"]`)?.insertAdjacentHTML("afterend",'<span class="discuss-badge discuss-badge-info" style="margin-left:0.375rem">Pinned</span>')):(n.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 17v5"/><path d="M9 10.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V16a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V7a1 1 0 0 1 1-1 2 2 0 0 0 0-4H8a2 2 0 0 0 0 4 1 1 0 0 1 1 1z"/></svg> <span>Pin</span>',document.querySelector(`#comment-${s} .discuss-badge-info`)?.remove()))}else window.app?.showToast("Failed to update pin.","error")}catch{window.app?.showToast("Network error.","error")}}async deleteComment(s){if(await window.app?.showConfirm({title:"Delete comment?",message:"This will permanently remove the comment. This cannot be undone.",confirmLabel:"Delete",isDanger:!0}))try{if((await fetch(`/api/admin/comments/${s}`,{method:"DELETE"})).ok){let t=document.getElementById(`comment-${s}`);t&&(t.style.transition="opacity 300ms ease, transform 300ms ease",t.style.opacity="0",t.style.transform="translateY(-8px)",setTimeout(()=>t.remove(),300)),window.app?.refreshPendingBadge()}else window.app?.showToast("Failed to delete comment.","error")}catch{window.app?.showToast("Network error.","error")}}};window.DiscussWidget=y;})();
