(()=>{var y={reply:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 18v-2a4 4 0 0 0-4-4H4"/><path d="m9 17-5-5 5-5"/></svg>',share:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg>',user:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',mail:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>',send:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/></svg>',edit:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>'};var T=document.currentScript,_=T?new URL(T.src).origin:window.location.origin;function A(d){let e=parseInt(d.replace("#",""),16);return[e>>16&255,e>>8&255,e&255]}function x(d,e,t){return"#"+[d,e,t].map(n=>Math.round(Math.max(0,Math.min(255,n))).toString(16).padStart(2,"0")).join("")}function I(d,e,t){d/=255,e/=255,t/=255;let n=Math.max(d,e,t),s=Math.min(d,e,t),r,i,a=(n+s)/2;if(n===s)r=i=0;else{let o=n-s;switch(i=a>.5?o/(2-n-s):o/(n+s),n){case d:r=((e-t)/o+(e<t?6:0))/6;break;case e:r=((t-d)/o+2)/6;break;default:r=((d-e)/o+4)/6}}return[r*360,i,a]}function E(d,e,t){d/=360;let n=(i,a,o)=>(o<0&&(o+=1),o>1&&(o-=1),o<1/6?i+(a-i)*6*o:o<1/2?a:o<2/3?i+(a-i)*(2/3-o)*6:i);if(e===0){let i=Math.round(t*255);return[i,i,i]}let s=t<.5?t*(1+e):t+e-t*e,r=2*t-s;return[n(r,s,d+1/3),n(r,s,d),n(r,s,d-1/3)].map(i=>Math.round(i*255))}function M(d,e){if(!d)return null;if(d=d.trim(),d.startsWith("var(")||d.startsWith("--")){let r=d.startsWith("var(")?d.slice(4,-1).split(",")[0].trim():d,i=document.createElement("span");i.style.cssText="position:fixed;left:-9999px;top:-9999px;color:var("+r+")",e.appendChild(i);let a=getComputedStyle(i).color;return e.removeChild(i),a?M(a,e):null}if(/^#[0-9a-fA-F]{6}$/.test(d))return d;if(/^#[0-9a-fA-F]{3}$/.test(d)){let[,r,i,a]=d.match(/^#(.)(.)(.)$/);return`#${r}${r}${i}${i}${a}${a}`}let t=d.match(/^rgba?\(\s*([\d.]+)[,\s]+([\d.]+)[,\s]+([\d.]+)/i);if(t)return x(+t[1],+t[2],+t[3]);let n=d.match(/^hsla?\(\s*([\d.]+)[,\s]+([\d.]+)%[,\s]+([\d.]+)%/i);if(n){let[r,i,a]=E(+n[1],+n[2]/100,+n[3]/100);return x(r,i,a)}let s=d.match(/^oklch\(\s*([\d.]+%?)[,\s]+([\d.]+)[,\s]+([\d.]+)/i);if(s){let r=parseFloat(s[1]);s[1].includes("%")&&(r/=100);let i=parseFloat(s[2]),a=parseFloat(s[3])*Math.PI/180,o=i*Math.cos(a),l=i*Math.sin(a),c=r+.3963377774*o+.2158037573*l,u=r-.1055613458*o-.0638541728*l,m=r-.0894841775*o-1.291485548*l,h=c**3,p=u**3,g=m**3,$=4.0767416621*h-3.3077115913*p+.2309699292*g,w=-1.2684380046*h+2.6097574011*p-.3413193965*g,v=-.0041960863*h-.7034186147*p+1.707614701*g,b=C=>C<=.0031308?12.92*C:1.055*C**(1/2.4)-.055;return x(Math.round(Math.max(0,Math.min(1,b($)))*255),Math.round(Math.max(0,Math.min(1,b(w)))*255),Math.round(Math.max(0,Math.min(1,b(v)))*255))}return null}function L(d){let[e,t,n]=A(d),[s,r]=I(e,t,n),i={50:.96,100:.93,200:.86,300:.74,400:.6,500:.48,600:.38,700:.3,800:.22,900:.15},a={};for(let[o,l]of Object.entries(i)){let[c,u,m]=E(s,Math.min(r,.85),l);a[o]=x(c,u,m)}return a}var f=class{constructor(e){window.DiscussWidgetInstance=this,this.container=e.container,this.container&&(this.postUrl=e.postUrl||this.container.dataset.url||window.location.pathname,this.serverUrl=e.serverUrl||_,this.fetchUrl=e.fetchUrl||`${this.serverUrl}/api/comments?post_url=${encodeURIComponent(this.postUrl)}`,this.configUrl=e.configUrl||`${this.serverUrl}/api/comments/config`,this.config={},this.primaryColor=e.primaryColor||null,this.domainId=e.domainId||null,this.title=e.title??"Leave a comment",this.placeholder=e.placeholder??"Share your thoughts... (*markdown* supported)",this.darkSelector=e.darkSelector||null,this.icons={name:e.icons?.name!==void 0?e.icons.name:y.user,email:e.icons?.email!==void 0?e.icons.email:y.mail,submit:e.icons?.submit!==void 0?e.icons.submit:y.send},this.editTokens=new Map,this._rawContent=new Map,this.init=this.init.bind(this),this.render=this.render.bind(this),this.renderComment=this.renderComment.bind(this),this.renderForm=this.renderForm.bind(this),this.handleSubmit=this.handleSubmit.bind(this),this.primaryColor&&this.applyTheme(this.primaryColor),this.watchPrimaryColor(),this.init())}applyTheme(e){let t=M(e,this.container);if(!t)return;let n=L(t),s=this.container;s.style.setProperty("--brand-50",n[50]),s.style.setProperty("--brand-100",n[100]),s.style.setProperty("--brand-200",n[200]),s.style.setProperty("--brand-300",n[300]),s.style.setProperty("--brand-400",n[400]),s.style.setProperty("--brand-500",n[500]),s.style.setProperty("--brand-600",n[600]),s.style.setProperty("--brand-700",n[700]),s.style.setProperty("--brand-800",n[800]),s.style.setProperty("--brand-900",n[900]),s.style.setProperty("--accent-fg",n[700]),s.style.setProperty("--accent-surface",n[50]),s.style.setProperty("--focus-ring",n[700])}watchPrimaryColor(){if(!this.primaryColor||!this.darkSelector)return;let e=this.primaryColor.trim();if(!e.startsWith("var(")&&!e.startsWith("--"))return;let t=()=>this.applyTheme(this.primaryColor),n=s=>{s&&new MutationObserver(t).observe(s,{attributes:!0,attributeFilter:["class"]})};n(document.documentElement),n(document.body)}injectDarkStyles(){if(!this.darkSelector)return;let e="discuss-dark-style";if(document.getElementById(e))return;let t=document.createElement("style");t.id=e,t.textContent=`${this.darkSelector} #discuss-comments {
        ...
        `,this.container.querySelectorAll("form[data-parent]").forEach(s=>{s.addEventListener("submit",this.handleSubmit)}),this.container.querySelectorAll(".discuss-reply-tag").forEach(s=>{s.addEventListener("click",r=>{r.preventDefault();let i=r.currentTarget.getAttribute("href"),a=i.startsWith("#")?i.substring(1):i,o=document.getElementById(a);if(o){o.scrollIntoView({behavior:"smooth",block:"center"});let l=o.style.backgroundColor,c=this.darkSelector?!!document.querySelector(this.darkSelector):!1;o.style.backgroundColor=c?"var(--accent-surface)":"var(--brand-50)",o.style.borderRadius="8px",setTimeout(()=>{o.style.transition="background-color 500ms ease",o.style.backgroundColor=l,setTimeout(()=>{o.style.transition="",o.style.borderRadius=""},500)},1500)}})}),this.container.querySelectorAll(".discuss-reply-btn").forEach(s=>{s.addEventListener("click",r=>{let i=r.currentTarget.dataset.id,a=document.getElementById(`discuss-reply-form-${i}`);a&&a.classList.toggle("discuss-hidden")})});let n=s=>{let r=s.currentTarget.dataset.id,i=!1,a=document.getElementById(`discuss-collapse-target-${r}`);a&&(i=a.classList.toggle("discuss-hidden"));let o=document.getElementById(`discuss-children-${r}`);o&&(a?o.classList.toggle("discuss-hidden",i):i=o.classList.toggle("discuss-hidden"));let l=document.querySelector(`.discuss-collapse-btn[data-id="${r}"]`);if(l){let c=l.querySelector("svg");c&&(c.style.transform=i?"rotate(-90deg)":"rotate(0deg)")}};this.container.querySelectorAll(".discuss-collapse-btn, .discuss-collapse-line").forEach(s=>{s.addEventListener("click",n)}),this.container.querySelectorAll(".discuss-edit-btn").forEach(s=>{s.addEventListener("click",r=>{let i=parseInt(r.currentTarget.dataset.id,10);this._startInlineEdit(i)})}),this.container.querySelectorAll(".discuss-share-btn").forEach(s=>{s.addEventListener("click",async r=>{let i=r.currentTarget,a=i.dataset.id,o=`${window.location.origin}${window.location.pathname}#comment-${a}`;if(navigator.share){try{await navigator.share({title:document.title,url:o})}catch(l){if(l.name==="AbortError")return;await this.copyToClipboard(i,o)}return}await this.copyToClipboard(i,o)})})}async copyToClipboard(e,t){try{if(navigator.clipboard)await navigator.clipboard.writeText(t);else{window.prompt("Copy link:",t);return}e.innerHTML=`${y.share} <span>Copied!</span>`,setTimeout(()=>{e.innerHTML=`${y.share} <span>Share</span>`},2e3)}catch{window.prompt("Copy link:",t)}}getInitialsColor(e){let t=["#0d4891","#16a34a","#b45309","#1e40af","#dc2626","#6b21a8","#be185d","#0369a1"],n=0;for(let s=0;s<e.length;s++)n=e.charCodeAt(s)+((n<<5)-n);return t[Math.abs(n)%t.length]}getAvatarHtml(e){let t=e.name?e.name.charAt(0).toUpperCase():"U";return`
        ...
        `}getAdminBadges(e){return""}getAdminTooltip(e){return""}getAdminControls(e){return""}getEditButton(e){return this.editTokens.has(e.id)?`<button class="discuss-action-btn discuss-edit-btn" data-id="${e.id}">${y.edit} <span>Edit</span></button>`:""}_startInlineEdit(e){let t=document.getElementById(`discuss-collapse-target-${e}`);if(!t)return;let n=t.querySelector(".discuss-comment-body"),s=t.querySelector(".discuss-flex.discuss-gap-2");if(!n||!s)return;let r=n.innerHTML,i=this._rawContent.get(e)||"",a=document.createElement("div");a.className="discuss-form-container";let o=document.createElement("textarea");o.className="discuss-form-textarea",o.value=i;let l=document.createElement("div");l.className="discuss-form-bottom",l.style.justifyContent="flex-end";let c=document.createElement("div");c.className="discuss-form-actions discuss-gap-2";let u=document.createElement("button");u.type="button",u.className="discuss-action-btn",u.textContent="Cancel";let m=document.createElement("button");m.type="button",m.className="discuss-btn discuss-btn-primary",m.textContent="Save",c.appendChild(u),c.appendChild(m),l.appendChild(c),a.appendChild(o),a.appendChild(l),n.innerHTML="",n.appendChild(a),s.style.display="none",o.focus(),o.setSelectionRange(o.value.length,o.value.length),m.addEventListener("click",()=>{this._saveEdit(e,o.value,m,n,s,r)}),u.addEventListener("click",()=>{n.innerHTML=r,s.style.display="",this.container.querySelectorAll(`.discuss-edit-btn[data-id="${e}"]`).forEach(h=>{h.addEventListener("click",()=>this._startInlineEdit(e))})})}async _saveEdit(e,t,n,s,r,i){let a=this.editTokens.get(e);if(!(!a||!t.trim())){n.disabled=!0,n.textContent="Saving\u2026";try{let o=await fetch(`${this.serverUrl}/api/comments/${e}`,{method:"PATCH",headers:{"Content-Type":"application/json","X-Edit-Token":a},credentials:"include",body:JSON.stringify({content:t.trim()})});if(o.ok)this._rawContent.set(e,t.trim()),this.init();else{let l=await o.json();alert(l.error||"Failed to save edit."),n.disabled=!1,n.textContent="Save"}}catch(o){console.error("[Discuss]",o),alert("Network error. Please try again."),n.disabled=!1,n.textContent="Save"}}}renderComment(e,t,n,s){t=t||0;let r=e.is_pinned?'<span class="discuss-badge discuss-badge-info" style="margin-left:0.375rem">Pinned</span>':"",i=e.is_author?'<span class="discuss-badge discuss-badge-success" style="margin-left:0.375rem">Author</span>':"",a=e.edited_at?'<span style="font-size:0.8125rem;color:var(--text-muted)">(edited)</span>':"",o=new Date(e.created_at).toLocaleDateString(void 0,{year:"numeric",month:"short",day:"numeric"}),l=this.getAdminBadges(e),c=this.getAdminTooltip(e),u=this.getAdminControls(e),m=n?`<a href="#comment-${s}" class="discuss-reply-tag"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m15 10 5 5-5 5"/><path d="M4 4v7a4 4 0 0 0 4 4h12"/></svg>${n}</a>`:"",h=`
        ...
        `,p="";if(e.children.length>0)if(t>=3){let v=e.children.map(b=>this.renderComment(b,3,e.name,e.id)).join("");p=`
                ...
                `;let g=e.content;m&&(g.startsWith("<p>")?g=g.replace("<p>",`<p>${m}`):g=m+g);let $=e.children.length>0&&t<3?`
        ...
        `;return t<3?w:w+p}renderForm(e){let t=this.config.honeypot_question?`<input type="text" name="honeypot_answer_given" placeholder="${this.config.honeypot_question}" style="display:none" tabindex="-1" autocomplete="off">`:"";return`
        ...
        `}async handleSubmit(e){e.preventDefault();let t=e.target,n=t.dataset.parent,s=t.querySelector('[type="submit"]');s.disabled=!0,s.innerHTML='<span class="discuss-spinner discuss-spinner-sm" style="margin-right:0.5rem"></span> Posting\u2026';let r={name:t.name.value.trim(),email:t.email.value.trim(),content:t.content.value.trim(),post_url:this.postUrl,parent_id:parseInt(n,10),honeypot_field:t.honeypot_field.value,honeypot_answer_given:t.honeypot_answer_given?t.honeypot_answer_given.value:void 0,...this.domainId?{domain_id:this.domainId}:{}};try{let i=await fetch(`${this.serverUrl}/api/comments`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify(r)});if(i.ok){let a=await i.json();if(a.editToken&&this.editTokens.set(a.id,a.editToken),t.reset(),parseInt(n,10)!==0){let o=document.getElementById(`discuss-reply-form-${n}`);o&&o.classList.add("discuss-hidden")}this.init()}else{let a=await i.json();alert(a.error||"Failed to post comment.")}}catch(i){console.error("[Discuss]",i),alert("Network error. Please try again.")}finally{s.disabled=!1,s.textContent="Post Comment"}}};window.DiscussWidget=f;var k=document.getElementById("discuss-comments");k&&k.dataset.isAdmin!=="true"&&new f({container:k,darkSelector:k.dataset.darkSelector||null});var S=class extends f{constructor(e){super(e),this.isAdmin=!0}getAdminBadges(e){return e.is_approved?"":'<span class="discuss-badge discuss-badge-warning" style="margin-left:0.375rem">Pending</span>'}getAdminTooltip(e){return e.email?`
            <span style="position:relative;display:inline-flex;align-items:center;color:var(--text-subtle);cursor:help;padding:0.125rem;line-height:0;margin-left:0.25rem"
                ...
                <span style="width:0.875rem;height:0.875rem;pointer-events:none;display:inline-flex">${y.mail}</span>
